...
REQUIRED_SNIPPETS = [
    ...
    "real-ga-evidence-audit-1.11.7.json",
...
REPORT_UPLOAD_SNIPPETS = [
    "upload-artifact",
    "dist/",
    "release/real-ga-evidence-audit-1.11.7.json",
...
def check(root: Path, *, require_real_workflows: bool = False) -> list[str]:
    ...
    fallback_paths = [root / "patches" / "v1.11.7-apply.patch", root / "Makefile"]
    ...
    if not text:
        issues.append("no workflow, Makefile, or v1.11.7 patch content found for governance validation")
        return issues
