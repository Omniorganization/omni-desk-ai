...
replace_once(
    ...
    '        roles_cfg = cast(dict[str, Any], acl.get("roles")) if isinstance(acl.get("roles"), dict) else {}\n'
    '        actors_cfg = cast(dict[str, Any], acl.get("actors")) if isinstance(acl.get("actors"), dict) else {}\n'
    '        tenants_cfg = cast(dict[str, Any], acl.get("tenants")) if isinstance(acl.get("tenants"), dict) else {}',
)
    ...
    async def _stream_with_provider(
        ...
        try:
            if callable(stream_fn):
                async for delta in cast(AsyncIterator[ModelDelta], stream_fn(safe)):
...
class ChatTurnService:
    ...
    def preflight_stream(self, *, request: Request, payload: dict[str, Any]) -> None:
        """Reject invalid streaming requests before response headers are sent."""
        self._content(payload)
        self._last_event_id(request)
        self.require_idempotency(self.cfg, request, payload)
        if not callable(getattr(self._router(), "stream", None)):
    ...
    async def _replay_cached(
        ...
        *,
        ...
        sequence += 1
        assistant_value = result.get("assistant_message")
        assistant: dict[str, Any] = (
            cast(dict[str, Any], assistant_value)
    ...
    async def stream(
        ...
        *,
        ...
        iterator = cast(AsyncGenerator[ModelDelta, None], stream_fn(model_request))
        pending: asyncio.Future[ModelDelta] | None = None
        completed = False
        try:
...
routes = routes.replace(
    ...
    '        conflict_policy_raw = str(payload.get("conflict_policy") or "manual-review")\n'
    "        if conflict_policy_raw not in {",
)
routes = routes.replace(
    '            raise HTTPException(422, "invalid conflict_policy")\n'
    '        operations = payload.get("operations") or []',
    '            raise HTTPException(422, "invalid conflict_policy")\n'
    '        conflict_policy = cast(Literal["server-wins", "client-wins", "manual-review", "merge"], conflict_policy_raw)\n'
    '        operations = payload.get("operations") or []',
)
...
new_chat_routes = '''    @app.post("/app/conversations/{conversation_id}/ask")
    ...
    async def api_chat_stream(request: Request):
        decision = await admin(request, "operator")
        payload = await request.json()
        chat_service.preflight_stream(request=request, payload=payload)
...
if not any(endpoint.get("path") == "/app/tasks/{task_id}/lease" for endpoint in contract.get("endpoints", [])):
    contract["endpoints"].append(
        {
            "method": "POST",
            "path": "/app/tasks/{task_id}/lease",
            "description": "Renew an active desktop runtime task lease using the current lease token.",
            "role": "operator",
            "client_surfaces": ["desktop"],
            "signed_device_required_in_production": ["desktop"],
        }
    )
...
executor_test = read("apps/desktop-tauri/tests/executor.test.ts")
old_capabilities = "['chat', 'local-runtime', 'shell_sandbox', 'dry_run']"
new_capabilities = (
    "['chat', 'local-runtime', 'shell_sandbox', 'file_operation', 'dry_run']"
)
if old_capabilities not in executor_test:
    raise RuntimeError("desktop capability assertion marker missing")
executor_test = executor_test.replace(old_capabilities, new_capabilities, 1)
write("apps/desktop-tauri/tests/executor.test.ts", executor_test)
...
replace_once(
    "apps/web-admin-next/app/layout.tsx",
...
# Keep the pre-existing tri-app integration test aligned with provider-native
# streaming.  The test double must implement the same contract as a real model
# router so the endpoint is exercised before any response bytes are emitted.
replace_once(
...
''',
    ...
    async def stream(self, request):
        ...
        yield ModelDelta(
            sequence=1,
            text_delta=f"assistant:{request.user}",
            provider="fake-provider",
            model="fake-chat-model",
            profile=request.metadata.get("profile", "fast"),
            usage_delta={"input_tokens": 2, "output_tokens": 5},
            finish_reason="stop",
    )
''',
)

# Bind the new streaming route into the existing per-surface typed contract
# matrices.  These cases exercise the real client method and keep the shared
# contract checker authoritative for all declared surfaces.
replace_once(
    ...
    "  { surface: 'web_admin', method: 'POST', contractPath: '/app/devices/enrollment/{enrollment_id}/verify', clientPath: '/api/omni/devices/enrollment/enroll-1/verify', invoke: (api) => api.verifyDeviceChallenge('enroll-1', 'challenge-1', 'web_1234567890abcdef1234567890abcdef1234', 'base64:sig') },\n  { surface: 'web_admin', method: 'POST', contractPath: '/api/chat/stream', clientPath: '/api/omni/chat/stream', invoke: (api) => api.streamChat('typed stream', { idempotencyKey: 'typed-web-stream', onEvent: () => undefined }) },\n];",
)
replace_once(
    ...
    "  { surface: 'desktop', method: 'POST', contractPath: '/app/devices/enrollment/{enrollment_id}/complete', clientPath: '/app/devices/enrollment/enroll-1/complete', invoke: (client) => client.completeEnrollment('enroll-1', 'pair-123', 'desktop-1', 'public-key') },\n  { surface: 'desktop', method: 'POST', contractPath: '/api/chat/stream', clientPath: '/api/chat/stream', invoke: (client) => client.streamChat('typed stream', { idempotencyKey: 'typed-desktop-stream', onEvent: () => undefined }) },\n  { surface: 'desktop', method: 'POST', contractPath: '/app/tasks/{task_id}/lease', clientPath: '/app/tasks/task-1/lease', signedInProduction: true, invoke: (client) => client.renewTaskLease('task-1', 'desktop-1', 'lease-token') },\n];",
)
replace_once(
    "apps/mobile-flutter/test/omni_api_test.dart",
    "    invoke: _invokeNotifications,\n  ),\n];",
    "    invoke: _invokeNotifications,\n  ),\n  TypedClientContractCase(\n    surface: 'mobile',\n    method: 'POST',\n    contractPath: '/api/chat/stream',\n    clientPath: '/api/chat/stream',\n    invoke: _invokeStreamChat,\n  ),\n];",
)
replace_once(
    "apps/mobile-flutter/test/omni_api_test.dart",
    "Future<dynamic> _invokeNotifications(OmniApiClient client) =>\n    client.notifications();",
    "Future<dynamic> _invokeNotifications(OmniApiClient client) =>\n    client.notifications();\nFuture<dynamic> _invokeStreamChat(OmniApiClient client) => client\n    .streamChat('typed stream', idempotencyKey: 'typed-mobile-stream')\n    .drain<void>();",
)
