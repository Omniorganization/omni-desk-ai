...
from omnidesk_agent.channels.ui_bridge import VisibleUIBridge
...
def test_ui_bridge_all_actions_and_app_guard():
    async def run_case():
        proxy = ToolProxy()
        tool = UIBridgeTool(UIBridgeConfig(enabled=True, allowed_apps=["Gmail"]), proxy)
...
def test_visible_ui_bridge_delegates_to_computer_actions():
    async def run_case():
        proxy = ToolProxy()
        bridge = VisibleUIBridge(UIBridgeConfig(enabled=True), proxy)
        assert bridge.name == "ui_bridge"
        assert (await bridge.observe_screen(ctx())).data["args"] == {"max_width": 1280}
        assert (await bridge.type_visible_reply("hello", ctx())).data["args"] == {"text": "hello"}
        assert (await bridge.press_send(ctx())).data["args"] == {"keys": ["enter"]}
        assert proxy.calls == [
            ("computer", "screenshot", {"max_width": 1280}),
            ("computer", "type_text", {"text": "hello"}),
            ("computer", "hotkey", {"keys": ["enter"]}),
        ]
