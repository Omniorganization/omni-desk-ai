...
def test_workflow_governance_accepts_patch_contract(tmp_path, capsys) -> None:
    patch = tmp_path / "patches" / "v1.12.7-apply.patch"
...
python scripts/check_release_configuration.py --scope external-ga-evidence
EXTERNAL_GA_EVIDENCE_EXPECTED_VERSION=1.12.7+root-monorepo-production-ga-candidate
python scripts/import_external_ga_evidence.py --write-report release/external-ga-evidence-import-report.json
...
python scripts/import_ios_real_device_evidence.py --write-report release/real-ga-evidence-audit-1.12.7.json
...
def test_workflow_governance_real_workflow_mode_rejects_patch_only(
    tmp_path, capsys
) -> None:
    patch = tmp_path / "patches" / "v1.12.7-apply.patch"
    patch.parent.mkdir(parents=True)
...
""",
    ...
    assert (
        "real workflow mode requires .github/workflows/release.yml"
        in capsys.readouterr().err
    )


def test_workflow_governance_real_workflow_mode_accepts_release_workflow(
    tmp_path, capsys
) -> None:
    workflows = tmp_path / ".github" / "workflows"
    workflows.mkdir(parents=True)
...
steps:
  ...
  - env:
      EXTERNAL_GA_EVIDENCE_EXPECTED_VERSION: 1.12.7+root-monorepo-production-ga-candidate
  - run: python scripts/check_release_configuration.py --scope external-ga-evidence
  - run: python scripts/import_external_ga_evidence.py --write-report release/external-ga-evidence-import-report.json
  - env:
      IOS_EVIDENCE_EXPECTED_VERSION: 1.12.7+root-monorepo-production-ga-candidate
  ...
  - run: python scripts/import_ios_real_device_evidence.py --write-report release/real-ga-evidence-audit-1.12.7.json
