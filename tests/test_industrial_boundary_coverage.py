...
def test_remote_sandbox_client_archive_post_and_error_paths(tmp_path, monkeypatch):
    ...
    (workspace / "too_large.bin").write_bytes(b"x" * 8)
    tight_client = RemoteSandboxClient("http://runner", max_file_bytes=4)
    with pytest.raises(RemoteSandboxError, match="file exceeds"):
        tight_client._archive_workspace(workspace)
    (workspace / "too_large.bin").unlink()

    if hasattr(os, "symlink"):
        (workspace / "linked.py").symlink_to(workspace / "keep.py")
        with pytest.raises(RemoteSandboxError, match="symlink"):
            client._archive_workspace(workspace)
        (workspace / "linked.py").unlink()
