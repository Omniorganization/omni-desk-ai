...
def _write_required_artifacts(package_dir: Path) -> None:
    files = [
        "Omni-desk-AI-1.12.5-core-release.zip",
        "Omni-desk-AI-1.12.5-web-admin.zip",
        "Omni-desk-AI-1.12.5-desktop.zip",
        "Omni-desk-AI-1.12.5-mobile.zip",
        f"{SLUG}-full.zip",
    ]
    for item in files:
...
def test_distribution_manifest_rejects_missing_required_artifact(tmp_path: Path, capsys) -> None:
    package_dir = tmp_path / "package"
    package_dir.mkdir()
    (package_dir / "Omni-desk-AI-1.12.5-core-release.zip").write_bytes(b"core")
    assert (
        main(
            [
                "--package-dir",
