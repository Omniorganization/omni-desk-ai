...
test('parseSseBlock accepts monotonic JSON events and ignores heartbeat', () => {
  ...
  assert.deepEqual(
    parseSseBlock('id: 3\nevent: chat.delta\ndata: {"text":"hello"}'),
    { id: 3, event: 'chat.delta', data: { text: 'hello' } },
  );
  assert.deepEqual(
    parseSseBlock('id: 4\nevent: chat.completed\ndata: {"native":true}'),
    { id: 4, event: 'chat.completed', data: { native: true } },
  );
...
test('Desktop stream sends signed resume and idempotency headers', async () => {
  ...
  try {
    ...
    assert.ok(observed);
    assert.equal(observed.headers.get('last-event-id'), '2');
    assert.equal(observed.headers.get('idempotency-key'), 'idem-1');
    assert.equal(observed.headers.get('x-omnidesk-device-signature'), 'signature');
  } finally {
