...
class UpdateAuditLog:
    ...
    def record(self, event: str, **payload: Any) -> None:
        with self.path.open("a", encoding="utf-8") as fh:
            fh.write(json.dumps({"event": event, **payload}, ensure_ascii=False, sort_keys=True) + "\n")
...
class UpdateManifestClient:
    ...
    def fetch_latest(self) -> ManifestBundle:
        ...
        if source.startswith("http://") or source.startswith("https://"):
            with urllib.request.urlopen(source, timeout=15) as response:  # nosec B310 - operator configured manifest URL
                raw = response.read()
            base_dir = None
        else:
...
class SignatureVerifier:
    """Verify update manifests with keyed Ed25519 release signatures only."""
    ...
    def verify_manifest(self, manifest: dict[str, Any]) -> SignatureVerification:
        ...
        for entry in signatures:
            ...
            if algorithm not in self.SUPPORTED_ALGORITHMS:
                last_error = f"unsupported manifest signature algorithm: {algorithm or 'missing'}"
                continue
            value = str(entry.get("value") or entry.get("signature") or "")
            try:
                if self._verify_ed25519(payload, value):
                    valid_count += 1
        ...
        return SignatureVerification(
            ...
            valid_count,
            algorithms,
            "" if valid_count else (last_error or "no Ed25519 signature matched release public key"),
        )
...
class ReleaseChannelResolver:
    ...
    def evaluate(self, *, manifest: dict[str, Any], current_version: str, cfg: UpdateConfig, signature: SignatureVerification) -> UpdatePolicyDecision:
        ...
        if not cfg.allow_background_download:
            return UpdatePolicyDecision(False, False, False, "background artifact download is disabled by local policy", channel)
...
class ArtifactDownloader:
    ...
    def download(self, artifact: dict[str, Any], *, base_dir: Path | None = None) -> Path:
        ...
        if not expected:
            raise RuntimeError("artifact sha256 is required by runtime update policy")
        if not SHA256_RE.fullmatch(expected):
            raise RuntimeError("artifact sha256 must be a lowercase 64-character hex digest")
...
class SBOMVerifier:
    def verify(self, *, manifest: dict[str, Any], artifact: dict[str, Any], base_dir: Path | None = None, require_sbom: bool = True) -> bool:
        ...
        sbom_sha = str(artifact.get("sbom_sha256") or (manifest.get("sbom") or {}).get("sha256") or "").strip().lower()
        if not sbom_path:
            if require_sbom:
                raise RuntimeError("SBOM is required by local update policy")
            return False
        if require_sbom and not sbom_sha:
            raise RuntimeError("SBOM sha256 is required by local update policy")
        if sbom_sha and not SHA256_RE.fullmatch(sbom_sha):
            raise RuntimeError("SBOM sha256 must be a lowercase 64-character hex digest")
        path = Path(str(sbom_path))
        if not path.is_absolute() and base_dir is not None:
            path = base_dir / path
        if not path.exists():
            raise RuntimeError("SBOM file is missing")
        if sbom_sha and sha256_file(path) != sbom_sha:
...
def default_release_health_check(candidate_dir: Path) -> bool:
    ...
    try:
        ...
        return False
    command = manifest.get("health_check_command") or manifest.get("health_check")
    if isinstance(command, list) and command:
        try:
            completed = subprocess.run(  # nosec B603 - command comes from a signed manifest
    ...
    for marker in ("VERSION", "pyproject.toml", "release_metadata.json", "omnidesk_agent", "apps", "bin"):
...
class AutoUpdateRunner:
    def __init__(self, *, cfg: UpdateConfig, manifest_client: UpdateManifestClient, signature_verifier: SignatureVerifier, health_check: Callable[[Path], bool] | None = None, audit: UpdateAuditLog | None = None):
        self.cfg = cfg
    ...
    def run_once(self, *, current_version: str) -> dict[str, Any]:
        ...
        health_ok = self.slots.activate(candidate_dir, health_check=self._combined_health_check if self.cfg.require_health_check else None, rollback_on_failure=self.cfg.rollback_on_failure)
        if not health_ok:
    ...
    def _combined_health_check(self, candidate_dir: Path) -> bool:
        if not default_release_health_check(candidate_dir):
            return False
        if self.health_check is None:
            return True
        return bool(self.health_check(candidate_dir))
