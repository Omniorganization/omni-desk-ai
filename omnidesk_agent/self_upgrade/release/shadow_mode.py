...
class ShadowComparison:
    task: str; stable_steps: int; shadow_steps: int; stable_risk: str; shadow_risk: str; improvement: str; recommendation: str; created_at: float
...
class ShadowModeEvaluator:
    ...
    def compare_plans(self, task: str, stable_plan: dict[str, Any], shadow_plan: dict[str, Any]) -> ShadowComparison:
        ...
        improvements=[]
        if shadow_steps < stable_steps: improvements.append("fewer steps")
        if self.RISK_ORDER.get(shadow_risk,1) < self.RISK_ORDER.get(stable_risk,1): improvements.append("lower risk")
        if not improvements: improvements.append("no clear improvement")
    ...
    def _max_risk(self, plan):
        ...
        for step in plan.get("steps",[]) or []:
            ...
            if self.RISK_ORDER.get(r,1)>self.RISK_ORDER.get(max_risk,0): max_risk=r
        return max_risk
