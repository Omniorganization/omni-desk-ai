...
class CanaryReleaseManager:
    ...
    def enable(self, target: str, version: str, allowed_risk: str="low") -> dict:
        state = self._load()
        state[target] = {"channel": "canary", "version": version, "allowed_risk": allowed_risk, "enabled_at": time.time()}
        self._save(state)
        return state[target]
    ...
    def disable(self, target: str) -> None:
        state = self._load()
        if target in state:
            state[target]["channel"] = "stable"
            state[target]["disabled_at"] = time.time()
        self._save(state)

    def should_use_canary(self, target: str, task_risk: str) -> bool:
        st = self._load().get(target)
