...
class ExperienceStore:
    ...
    def _init(self) -> None:
        ...
        CREATE TABLE IF NOT EXISTS structured_experiences (
          ...
          namespace TEXT NOT NULL DEFAULT 'unknown:unknown'
        )
        """)
        for name, ddl in {
            ...
            "channel": "ALTER TABLE structured_experiences ADD COLUMN channel TEXT NOT NULL DEFAULT 'unknown'",
            "actor": "ALTER TABLE structured_experiences ADD COLUMN actor TEXT NOT NULL DEFAULT 'unknown'",
            "namespace": "ALTER TABLE structured_experiences ADD COLUMN namespace TEXT NOT NULL DEFAULT 'unknown:unknown'",
        }.items():
        ...
        CREATE TABLE IF NOT EXISTS interaction_profiles (
          ...
          preferred_surface TEXT NOT NULL DEFAULT 'local_gateway',
    ...
    def search_similar(self, query: str, limit: int = 5, *, only_reusable: bool = False) -> list[dict[str, Any]]:
        ...
        if self.encryption.enabled:
            if only_reusable:
                rows = cur.execute(
                    "SELECT * FROM structured_experiences WHERE reusable_skill = 1 ORDER BY updated_at DESC LIMIT ?",
                    (max(limit * 10, limit),),
                ).fetchall()
            else:
                rows = cur.execute(
                    "SELECT * FROM structured_experiences ORDER BY updated_at DESC LIMIT ?",
                    (max(limit * 10, limit),),
                ).fetchall()
        ...
        try:
            if only_reusable:
                rows = cur.execute(
                    ...
                    WHERE structured_experiences_fts MATCH ? AND e.reusable_skill = 1
                    ORDER BY rank
                    LIMIT ?
                    """,
                    (query, limit),
                ...
                rows = cur.execute(
                    ...
                    SELECT e.* FROM structured_experiences_fts f
                    JOIN structured_experiences e ON e.id = f.rowid
                    WHERE structured_experiences_fts MATCH ?
                    ORDER BY rank
                    LIMIT ?
                    """,
                    (query, limit),
    ...
    def list_structured(self, *, days: int = 30, limit: int = 200, statuses: Optional[list[str]] = None) -> list[dict[str, Any]]:
        ...
        params.append(limit)
        # Dynamic SQL is limited to generated placeholders; all values remain bound parameters.
        rows = self.conn.execute(
            f"""
    ...
    def purge_expired(
        ...
        *,
        ...
        """Purge structured memories whose retention deadline has passed.

        The method is deliberately scoped to structured memories because they
        carry explicit retention metadata. It supports dry-run reporting for
        operator review before destructive deletion and writes an audit event for
        both dry-run and actual purge decisions.
        """
        cutoff = time.time() if now is None else float(now)
        limit = max(1, min(int(limit), 10000))
        filters = ["expires_at IS NOT NULL", "expires_at <= ?"]
        params: list[Any] = [cutoff]
        namespace_filter_applied = False
        ...
        if actor is not None:
            ...
            namespace_filter_applied = True
        params.append(limit)
        # Filter fragments are static literals; caller values remain bound parameters.
        rows = self.conn.execute(
            f"""
            SELECT id, created_at, updated_at, task_type, risk_level, memory_status,
                   privacy_level, expires_at, channel, actor, namespace
            FROM structured_experiences
        ...
        if ids and not dry_run:
            ...
            # Placeholder count is generated from already selected integer ids.
            self.conn.execute(f"DELETE FROM structured_experiences WHERE id IN ({placeholders})", ids)  # nosec B608
        ...
        event = {
            ...
            "channel": channel,
            "actor": actor,
            "namespace_filter_applied": namespace_filter_applied,
        }
    ...
    def record_interaction_profile(
        self,
        *,
        channel: str,
        actor: str,
        ...
        from omnidesk_agent.learning.interaction_profile import empty_profile, infer_interaction_signal, profile_from_row

        channel = str(channel or "unknown")
        actor = str(actor or "unknown")
        namespace = f"{channel}:{actor}"
        existing_row = self.conn.execute("SELECT * FROM interaction_profiles WHERE namespace = ?", (namespace,)).fetchone()
        existing = self._decode_interaction_profile(dict(existing_row)) if existing_row else None
        signal = infer_interaction_signal(task, source_channel=channel, actor=actor, learned_profile=existing)
        success = status == "completed"
        now = time.time()

        preferred_surface = signal.surface
        preferred_channel = signal.target_channel or existing.get("preferred_channel", "unknown") if existing else (signal.target_channel or "unknown")
        preferred_app = signal.ui_bridge_app or existing.get("preferred_app") if existing else signal.ui_bridge_app
        if existing and not success:
            preferred_surface = str(existing.get("preferred_surface") or preferred_surface)
            preferred_channel = str(existing.get("preferred_channel") or preferred_channel)
            preferred_app = existing.get("preferred_app") or preferred_app
        ...
        values = {
            ...
            "manual_intervention_count": 1 if manual_intervention else 0,
            "safety_violation_count": 1 if safety_violation else 0,
        ...
        if existing is None:
            self.conn.execute(
                ...
                INSERT INTO interaction_profiles (
                    namespace, channel, actor, task_count, success_count, failure_count,
                    manual_intervention_count, safety_violation_count, preferred_surface,
                    preferred_channel, preferred_app, last_task, last_status, updated_at
                ) VALUES (
                    :namespace, :channel, :actor, :task_count, :success_count, :failure_count,
                    :manual_intervention_count, :safety_violation_count, :preferred_surface,
                    :preferred_channel, :preferred_app, :last_task, :last_status, :updated_at
            ...
            self.conn.execute(
                ...
                SET task_count = task_count + :task_count,
                    success_count = success_count + :success_count,
                    failure_count = failure_count + :failure_count,
                    manual_intervention_count = manual_intervention_count + :manual_intervention_count,
                    safety_violation_count = safety_violation_count + :safety_violation_count,
                    preferred_surface = :preferred_surface,
                    preferred_channel = :preferred_channel,
                    preferred_app = :preferred_app,
                    last_task = :last_task,
        ...
        self._audit_memory_governance({
            "event": "interaction_profile_update",
            "namespace": namespace,
            "channel": channel,
            "actor": actor,
            "status": status,
        ...
        self.conn.commit()
        row = self.conn.execute("SELECT * FROM interaction_profiles WHERE namespace = ?", (namespace,)).fetchone()
        if row is None:
            return empty_profile(channel, actor)
        return profile_from_row(self._decode_interaction_profile(dict(row)))

    def get_interaction_profile(self, channel: str, actor: str) -> dict[str, Any]:
        from omnidesk_agent.learning.interaction_profile import empty_profile, profile_from_row

        channel = str(channel or "unknown")
        actor = str(actor or "unknown")
        row = self.conn.execute(
            "SELECT * FROM interaction_profiles WHERE namespace = ?",
            (f"{channel}:{actor}",),
        ).fetchone()
        if row is None:
            return empty_profile(channel, actor)
    ...
    def _decode_interaction_profile(self, row: dict[str, Any]) -> dict[str, Any]:
        if "last_task" in row:
            row["last_task"] = self._decrypt_text(row["last_task"])
        return row
