...
from omnidesk_agent.server_routes.agent_routes import (
    register_agent_routes,
    register_break_glass_routes,
)
...
def create_app(cfg: AppConfig) -> FastAPI:
    ...
    resource_guard = ChatAwareApiResourceGuard(cfg.api_resource_guard)
    otel_endpoint = os.getenv(cfg.observability.otlp_endpoint_env, "")
    otel_exporter = (
        AsyncOTLPHttpExporter(
            endpoint=otel_endpoint,
            timeout=cfg.observability.otlp_timeout_seconds,
        )
        if otel_endpoint
        else None
    )
    ...
    async def request_context_middleware(request: Request, call_next):
        request_id = (
            request.headers.get(cfg.observability.request_id_header) or new_request_id()
        )
        ...
        try:
            ...
            with trace_span(
                ...
                response.headers["traceparent"] = make_traceparent(
                    span.trace_id,
                    span.span_id,
                )
                return response
