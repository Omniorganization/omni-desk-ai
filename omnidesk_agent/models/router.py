...
class ModelRouter:
    def __init__(
        ...
        cost_store: ModelCostStore | None = None,
        *,
        ...
        self.providers = {
            name: self._build_provider(name, p)
        ...
        if require_persistent_ledger and cost_store is None:
            raise RuntimeError(
                "models.budget.require_persistent_ledger requires a configured model cost_store"
            )
    ...
    def _build_budget_enforcer(
        self, cost_store: ModelCostStore | None
    ) -> ModelBudgetEnforcer | None:
        ...
        policy = ModelBudgetPolicy(
            ...
            per_actor_daily_usd_limit=getattr(
                budget_cfg, "per_actor_daily_usd_limit", None
            ),
            on_exceed=getattr(budget_cfg, "on_exceed", "require_approval"),
        )
        if (
            policy.daily_usd_limit is None
            and policy.monthly_usd_limit is None
            and policy.per_actor_daily_usd_limit is None
        ):
            return None
        return ModelBudgetEnforcer(cost_store, policy)

    def _check_budget(
        self, request: ModelRequest, *, profile: str, provider: Any
    ) -> str | None:
        if self.budget_enforcer is None:
            return None
        projected = self._estimate_projected_cost(request, provider)
        actor = request.metadata.get("actor")
        decision = self.budget_enforcer.check(
            actor=str(actor) if actor else None, projected_cost_usd=projected
        )
        if decision.ok:
            return None
        if (
            decision.action == "fallback_local"
            and profile != "local"
        ...
        raise RuntimeError(
            ...
            f"{decision.reason}; action={decision.action}; "
            f"observed={decision.observed_cost_usd:.6f}; limit={decision.limit_usd}"
        )

    def _estimate_projected_cost(self, request: ModelRequest, provider: Any) -> float:
        output_tokens = int(
            getattr(provider.settings, "max_output_tokens", self.cfg.max_output_tokens)
            or self.cfg.max_output_tokens
        )
        return self.pricing_table.estimate(
            provider=str(getattr(provider, "provider_name", "")),
            model=str(getattr(provider, "model", "")),
            input_tokens=self.token_budget.estimate_tokens(request.system)
            + self.token_budget.estimate_tokens(request.user),
    ...
    def _offline_forbids_profile(self, profile: str, provider: Any) -> bool:
        ...
        provider_name = str(
            ...
            or getattr(getattr(provider, "settings", None), "provider", "")
        )
    ...
    def _assert_explicit_profile_allowed(
        self, profile: str, metadata: dict[str, Any]
    ) -> None:
        ...
        role_allowed = any(
            _profile_matches(profile, roles_cfg.get(role))
            for role in roles
        )
        actor_allowed = (
            actor
            and actor in actors_cfg
            and _profile_matches(profile, actors_cfg.get(actor))
        )
        if tenant and tenant in tenants_cfg and not _profile_matches(
            profile, tenants_cfg.get(tenant)
        ):
            raise PermissionError(
                f"model profile denied by tenant ACL: profile={profile}"
            )
        ...
        approved_profiles = _metadata_values(
            metadata,
            approval_key,
        ...
        if profile not in approved_profiles and "*" not in approved_profiles:
            raise PermissionError(
                f"high-cost model profile requires approval: profile={profile}"
            )
    ...
    def _record_failure(self, profile: str) -> None:
        state = self._circuit.setdefault(profile, {"failures": 0, "opened_at": 0.0})
