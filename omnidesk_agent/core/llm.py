...
class OpenAIChatLLM(RouterLLMAdapter):
    def __init__(self, cfg: LLMConfig, token_budget: Optional[TokenBudgetManager]=None):
        from omnidesk_agent.config import ModelsConfig, ModelProfileConfig
        if token_budget is None:
            raise RuntimeError('OpenAIChatLLM now requires TokenBudgetManager through ModelRouter')
