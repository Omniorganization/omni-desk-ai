...
from omnidesk_agent.appsync.chat_service import (
    ChatStreamEvent,
    ChatTurnService,
    PreparedChatTurn,
)
from omnidesk_agent.appsync.conversation_context import (
...
def _lease_seconds(cfg: Any) -> int:
    configured = getattr(
        getattr(cfg, "app_sync", None),
        "chat_request_lease_seconds",
        180,
    )
    raw = os.getenv("OMNIDESK_APPSYNC_CHAT_LEASE_SECONDS", str(configured))
    try:
...
class IndustrialChatTurnService(ChatTurnService):
    ...
    def _reserve(
        ...
        *,
        ...
        try:
            ...
            raise HTTPException(
                409,
                {"code": "stream_in_progress", "message": str(exc)},
            ) from exc
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "chat failure result discarded after lease loss",
                    extra={
                        "trace_id": trace_id,
                        "conversation_id": reservation.conversation_id,
                    },
        ...
        try:
            ...
            raise HTTPException(
                409,
                {"code": "chat_request_lease_lost", "message": str(exc)},
            ) from exc
        if self.metrics:
    ...
    def _append_stream_event(
        ...
        *,
        ...
        if event.event == "chat.usage" and mapped == "running":
            mapped = "finalizing"
        self.atomic_repository.append_event(
            reservation,
    ...
    async def stream_prepared(
        ...
        *,
        ...
        try:
            async for event in super().stream_prepared(
                request=request,
                prepared=prepared,
                actor=actor,
                last_event_id=last_event_id,
