...
def create_appsync_store(cfg: Any) -> AppSyncStore:
    ...
    if backend == "postgres":
        ...
        return ApplyingDurablePostgresAppSyncStore(dsn=dsn, namespace=namespace, local_outbox_enabled=bool(getattr(getattr(cfg, "runtime", None), "offline_mode", False)))

    path = getattr(app_sync_cfg, "json_path", None) if app_sync_cfg is not None else None
    if path:
        return ApplyingAppSyncStore(path, local_outbox_enabled=bool(getattr(getattr(cfg, "runtime", None), "offline_mode", False)))
    return ApplyingAppSyncStore(cfg.workspace.root / "app_sync_state.json", local_outbox_enabled=bool(getattr(getattr(cfg, "runtime", None), "offline_mode", False)))
