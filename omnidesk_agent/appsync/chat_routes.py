...
from omnidesk_agent.appsync.chat_service import ChatStreamEvent
from omnidesk_agent.appsync.factory import create_appsync_store
from omnidesk_agent.appsync.industrial_chat_service import IndustrialChatTurnService as ChatTurnService
...
def register_first_class_chat_routes(app: FastAPI, cfg: Any, runtime: Any, metrics: Any, admin: Any) -> ChatTurnService:
    """Register canonical chat routes before legacy AppSync route collections."""
    store = _store(runtime, cfg)
    service = ChatTurnService(cfg=cfg, runtime=runtime, store=store, metrics=metrics)
    ...
    async def app_chat_turn(conversation_id: str, request: Request):
        ...
        return await service.complete(
            request=request, payload=payload, actor=_actor(decision),
            role=str(getattr(decision, "role", "operator")),
            conversation_id=conversation_id, default_title="Conversation chat",
        )
    ...
    async def api_chat_stream(request: Request):
        ...
        stream_service = ChatTurnService(cfg=cfg, runtime=runtime, store=store, metrics=metrics)
        stream_service.stream_limit = service.stream_limit
        active_router = getattr(runtime, "model_router", None)
        if active_router is not None and not callable(getattr(active_router, "route_plan", None)):
            stream_service.streaming_router = CompletionOnlyStreamingRouter(active_router)
        ...
        async def events():
            queue: asyncio.Queue[ChatStreamEvent | BaseException | None] = asyncio.Queue(maxsize=STREAM_QUEUE_SIZE)

            async def produce() -> None:
                cancelled = False
                try:
                    async for event in stream_service.stream_prepared(
                        request=request, prepared=prepared, actor=actor,
                        last_event_id=last_event_id,
                    ):
            ...
            try:
                while True:
                    ...
                    if isinstance(item, BaseException):
                        ...
                        detail = (
                            item.detail if isinstance(item, HTTPException) and isinstance(item.detail, dict)
                            else {"code": "chat_stream_failed"}
                        )
                        yield _encode(ChatStreamEvent(failure_sequence, "chat.failed", detail))
                        break
