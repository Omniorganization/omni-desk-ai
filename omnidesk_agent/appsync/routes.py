...
def _is_production() -> bool:
    return os.getenv("OMNIDESK_ENV", "").strip().lower() == "production"
...
def _is_predictable_device_id(value: str) -> bool:
    ...
    if not lowered:
        return False
    banned = {
        "desktop", "desktop-1", "desktop-runtime", "desktop-device",
        "mobile", "mobile-1", "mobile-device", "flutter-mobile",
        "test-device", "demo-device", "sample-device", "web-admin-console",
    }
    return lowered in banned or lowered.startswith("test-") or lowered.startswith("demo-")
...
def _device_signature_enforced(cfg: Any) -> bool:
    app_sync = getattr(cfg, "app_sync", None)
    return _is_production() and bool(getattr(app_sync, "require_device_signed_requests_in_production", True))
...
async def _require_signed_device_request(
    *,
    ...
    device_id = str(headers.get("x-omnidesk-device-id") or expected_device_id or "").strip()
    if expected_device_id and device_id != expected_device_id:
        if metrics:
            metrics.inc("omnidesk_device_signature_failures_total", reason="device_id_mismatch")
    ...
    ok, reason = store.verify_device_request_signature(
        ...
        max_skew_seconds=int(getattr(getattr(cfg, "app_sync", None), "device_signature_max_skew_seconds", 300)),
        nonce_ttl_seconds=int(getattr(getattr(cfg, "app_sync", None), "device_request_nonce_ttl_seconds", 600)),
    )
    if not ok:
        if metrics:
    ...
    if metrics:
        metrics.inc("omnidesk_device_signed_requests_total", surface="app", device_type=','.join(sorted(required_device_types)))


def _websocket_headers(websocket: WebSocket, cfg: Any) -> dict[str, str]:
    headers = {k.lower(): v for k, v in websocket.headers.items()}
    app_sync = getattr(cfg, "app_sync", None)
    allow_query_auth = bool(getattr(app_sync, "allow_websocket_query_auth", False)) and not _is_production()
...
def register_appsync_routes(app: FastAPI, cfg: Any, rt: Any, metrics: Any, admin: Any) -> None:
    ...
    async def _complete_chat_turn(conversation_id: str, request: Request, payload: dict[str, Any], actor: str) -> dict[str, Any]:
        content = str(payload.get("content") or payload.get("message") or "").strip()
        if not content:
            raise HTTPException(422, "content is required")
        if bool(payload.get("stream", False)):
            raise HTTPException(422, "streaming chat is not enabled for this endpoint; use /api/chat/stream")
    ...
    async def app_bootstrap(request: Request):
        ...
        metrics.inc("omnidesk_app_requests_total", surface="shared", operation="bootstrap") if metrics else None
    ...
    async def app_register_device(request: Request):
        ...
        if _is_production() and device_type in {"desktop", "mobile"}:
            if getattr(app_sync_cfg, "require_device_public_key_in_production", True) and not public_key:
    ...
    async def app_ask_conversation(conversation_id: str, request: Request):
        decision = await admin(request, "operator")
        payload = await request.json()
        return await _complete_chat_turn(conversation_id, request, payload, _actor(decision))
    ...
    async def api_chat_stream(request: Request):
        ...
        raise HTTPException(501, "streaming chat is not enabled in the source-gated candidate; use /api/chat for audited non-streaming chat")
    ...
    async def app_update_task_status(task_id: str, request: Request):
        ...
        await _require_signed_device_request(cfg=cfg, store=store, request=request, raw_body=raw_body, required_device_types={"desktop"}, expected_device_id=str(payload.get("assigned_runtime_device_id") or request.headers.get("x-omnidesk-device-id") or ""), metrics=metrics)
        if status not in {"queued", "running", "blocked", "completed", "failed", "cancelled"}:
            raise HTTPException(422, "invalid task status")
        try:
            task = store.update_task_status(task_id=task_id, actor=_actor(decision), status=status, result_summary=payload.get("result_summary"), assigned_runtime_device_id=payload.get("assigned_runtime_device_id"), idempotency_key=_require_idempotency(cfg, request, payload), idempotency_payload=payload)
        except IdempotencyConflict as exc:
            raise HTTPException(409, str(exc)) from exc
        except KeyError as exc:
            raise HTTPException(404, str(exc)) from exc
    ...
    async def app_decide_approval(approval_id: str, request: Request):
        ...
        payload, raw_body = await _json_body_and_raw(request)
        await _require_signed_device_request(cfg=cfg, store=store, request=request, raw_body=raw_body, required_device_types={"mobile", "web_admin"}, expected_device_id=str(payload.get("source_device_id") or request.headers.get("x-omnidesk-device-id") or ""), metrics=metrics)
        verdict = payload.get("decision")
        if verdict not in {"approved", "rejected"}:
            raise HTTPException(422, "decision must be approved or rejected")
        try:
            approval = store.decide_approval(approval_id=approval_id, actor=_actor(decision), decision=verdict, reason=payload.get("reason"), idempotency_key=_require_idempotency(cfg, request, payload), idempotency_payload=payload)
        except IdempotencyConflict as exc:
            raise HTTPException(409, str(exc)) from exc
        except KeyError as exc:
            raise HTTPException(404, str(exc)) from exc
    ...
    async def app_register_push_token(device_id: str, request: Request):
        decision = await admin(request, "operator")
        payload, raw_body = await _json_body_and_raw(request)
        await _require_signed_device_request(cfg=cfg, store=store, request=request, raw_body=raw_body, required_device_types={"desktop", "mobile"}, expected_device_id=device_id, metrics=metrics)
    ...
    async def app_rotate_device_token(device_id: str, request: Request):
        ...
        await _require_signed_device_request(cfg=cfg, store=store, request=request, raw_body=raw_body, required_device_types={"desktop", "mobile", "web_admin"}, expected_device_id=device_id, metrics=metrics)
        try:
    ...
    async def app_revoke_device(device_id: str, request: Request):
        decision = await admin(request, "owner")
        payload, raw_body = await _json_body_and_raw(request)
        await _require_signed_device_request(cfg=cfg, store=store, request=request, raw_body=raw_body, required_device_types={"web_admin", "mobile"}, expected_device_id=str(request.headers.get("x-omnidesk-device-id") or ""), metrics=metrics)
        try:
    ...
    async def app_ws(websocket: WebSocket):
        ...
        auth_decision = rt.admin_auth.verify_headers(headers, client_host=client_host, required_role="viewer", path="/app/ws")
