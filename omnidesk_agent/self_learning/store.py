...
class SelfLearningStore:
    ...
    TABLES = {
        ...
        "self_learning_approval_history",
    ...
    def _init(self) -> None:
        ...
        """)
        self.conn.execute("""
        CREATE TABLE IF NOT EXISTS self_learning_approval_history (
          id TEXT PRIMARY KEY,
          created_at REAL NOT NULL,
    ...
    def save_approval(self, approval: Any) -> None:
        ...
        self._append_approval_history(payload)
        self.conn.commit()
    ...
    def _append_approval_history(self, payload: dict[str, Any]) -> None:
        history_created_at = time.time()
        history_id = f"{payload['approval_id']}:{payload['status']}:{history_created_at:.9f}"
        self.conn.execute(
            """
            INSERT INTO self_learning_approval_history
              (id, created_at, approval_id, proposal_id, approval_type, status, reviewer, decided_at, payload_json)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                history_id,
