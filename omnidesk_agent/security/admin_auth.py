    ...
    def verify_headers(self, headers: Any, client_host: Optional[str] = None, required_role: str = "viewer", path: str = "") -> AdminAuthDecision:
        ...
        if configured_tokens:
            ...
            if matching:
                ...
                decision = self._role_decision(True, "admin token accepted", f"token:{env_name}", role, required_role)
                self._audit(decision, client_host, required_role, path)
                return decision
            ...
            decision = self._role_decision(True, "legacy gateway secret accepted", "token:legacy-gateway-secret", "owner", required_role)
            self._audit(decision, client_host, required_role, path)
            return decision
