...
from omnidesk_agent.security.command_policy import SAFE_CI_ALLOWED_PREFIXES, argv_allowed
from omnidesk_agent.sandbox.limits import DEFAULT_MAX_ARCHIVE_BYTES, DEFAULT_MAX_ARCHIVE_FILE_BYTES, DEFAULT_MAX_ARCHIVE_FILES


ALLOWED_PREFIXES = SAFE_CI_ALLOWED_PREFIXES
...
class RunnerConfig:
    ...
    max_archive_bytes: int = int(os.getenv("OMNIDESK_SANDBOX_MAX_ARCHIVE_BYTES", str(DEFAULT_MAX_ARCHIVE_BYTES)))
    ...
    max_archive_files: int = int(os.getenv("OMNIDESK_SANDBOX_MAX_ARCHIVE_FILES", str(DEFAULT_MAX_ARCHIVE_FILES)))
    max_archive_file_bytes: int = int(os.getenv("OMNIDESK_SANDBOX_MAX_ARCHIVE_FILE_BYTES", str(DEFAULT_MAX_ARCHIVE_FILE_BYTES)))
...
def _allowed(argv: list[str], payload: dict[str, Any] | None = None) -> bool:
    if argv_allowed(argv, ALLOWED_PREFIXES):
        return True
    payload = payload or {}
    if str(payload.get("purpose") or "") == "plugin":
        # Plugin execution is still sandboxed by image allowlist, read-only bind
        # mount, no network, non-root user and signature-verified plugin manifests.
        return len(argv) == 3 and argv[:2] == ["python3", "-I"] and "/" not in argv[2] and ".." not in argv[2]
    return False
...
def _init_nonce_db(path: Path) -> None:
    ...
    with connect_sqlite(path, timeout=5, isolation_level="DEFERRED") as conn:
...
def _consume_nonce(nonce: str, now: float, cfg: RunnerConfig) -> bool:
    if cfg.nonce_db_path is not None:
        with _NONCE_LOCK:
            ...
            with connect_sqlite(cfg.nonce_db_path, timeout=5, isolation_level="DEFERRED") as conn:
...
class SandboxRunnerHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            argv = [str(x) for x in payload.get("argv") or []]
            if not argv or not _allowed(argv, payload):
            ...
            try:
                ...
                stdin_raw = b""
                if payload.get("stdin_base64"):
                    stdin_raw = base64.b64decode(str(payload.get("stdin_base64")).encode(), validate=True)
                    if len(stdin_raw) > cfg.max_body_bytes:
                        raise ValueError("stdin payload exceeds maximum size")
                result = subprocess.run(cmd, input=stdin_raw.decode("utf-8", errors="replace") if stdin_raw else None, text=True, capture_output=True, timeout=timeout, check=False)
...
def run_server(host: str = "0.0.0.0", port: int = 18890, cfg: RunnerConfig | None = None) -> None:  # nosec B104
...
if __name__ == "__main__":
    run_server(host=os.getenv("OMNIDESK_SANDBOX_RUNNER_HOST", "0.0.0.0"), port=int(os.getenv("OMNIDESK_SANDBOX_RUNNER_PORT", "18890")))  # nosec B104
