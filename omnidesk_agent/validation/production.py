...
def validate_production_config(cfg: AppConfig, environ: Optional[Mapping[str, str]] = None) -> dict:
    ...
    if cfg.gateway.public_base_url and str(cfg.gateway.public_base_url).strip().lower().rstrip("/") in PLACEHOLDER_URLS:
    ...
    if cfg.channels.chrome.enabled and not cfg.channels.chrome.allowed_origins:
